...
def run(args: list, env: Dict[str, str] = None) -> str:
    result = subprocess.run(args, check=True, stdout=subprocess.PIPE, text=False, env=env)
...
def default_repo() -> str:
    ...
    for remote in ("origin", "upstream"):
        ...
        if repo:
            return repo
    # 未找到 GitHub/Gitee remote 时返回空字符串，调用方将回退到 local_pr_event
    return ""
